...
def db_migrate():
    ...
    if result_migrate.returncode == 0:
        ...
        click.echo(
            click.style(
                "Note: No new migrations needed or an error occurred " "while generating migrations.", fg='yellow'
            )
        )
    ...
    if result_upgrade.returncode == 0:
        ...
        click.echo(
            click.style(
                "Error applying migrations. This may be due to the database " "being already up-to-date.", fg='yellow'
            )
        )
